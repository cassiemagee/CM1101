...
def print_menu(exits):
    ...
    for direction in exits:
        leadsTo = exit_leads_to(exits, direction)
        print_menu_line(direction, leadsTo)
...
def menu(exits):
    ...
    while True:
        ...
        # Read player's input
        # Normalise the input
        print("Please enter the direction you would like to go. ")
        imp = input("")
        normalise_input(imp)
       # imp = normalise_input(input(""))

        # Check if the input makes sense (is valid exit)
        if is_valid_exit(exits, imp) == True:
            # If so, return the player's choice
            return imp
        elif is_valid_exit(exits, imp) == False:
            print("Invalid Input. Please try again ")
            imp2 = normalise_input(input(""))
        else:
            break
